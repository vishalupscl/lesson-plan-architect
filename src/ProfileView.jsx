...
export default function ProfileView({ subjects, taxonomies, callLlm, showToast, masterPrompt }) {
...
function ProfileEditor({ entry, taxonomies, callLlm, showToast, masterPrompt, onSave, onClose }) {
  ...
  const extractSection = async (section) => {
    ...
    setBusy(true);
    try {
      const out = await callLlm(buildExtractPrompt(section, text), 700);
  ...
  const addStruggle = async () => {
    ...
    try {
      ...
      const out = await callLlm(buildStrugglePrompt(s, p.subject, conceptList), 400);
